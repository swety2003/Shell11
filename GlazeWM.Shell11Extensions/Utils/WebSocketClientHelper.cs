...
namespace GlazeWM.Shell11Extensions.Utils
{
    public class WebSocketClientHelper : IDisposable
    {
        ...
        public bool Send(string mess)
        {
            ...
            Task.Run(async () =>
            {
                var replyMess = Encoding.Default.GetBytes(mess);
                //发送消息
                await ws.SendAsync(replyMess, WebSocketMessageType.Text, true, CancellationToken.None);
            });
